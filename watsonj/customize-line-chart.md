# Customize the Line Chart

In this procedure, I will show you how to customize the line chart that you previously made by adding titles, axis labels, etc.

1. **Import Matplotlib into Python**

![Python6](assets/images/screenshots/Python6.png)


2. **Define your data**
Define your data using x and y values.

![Python7](assets/images/screenshots/Python7.png)

3. **Create the line plot**
Create the line plot using "plt.plot()"

![Python8](assets/images/screenshots/Python8.png)

4. Add a title to your plot
Add a title to your plot using "plt.title()"

![Python11](assets/images/screenshots/Python11.png)


5. **Add axis labels**
Add axis labels using "plt.xlabel()" and "plt.ylabel()" respectively.

![Python12](assets/images/screenshots/Python12.png)

6. **Add lines on the grid**
Add a line grid to the plot using "plt.grid()"

![Python14](assets/images/screenshots/Python13.png)

7. **Display your plot**
Display your plot using "plt.show()"

![Python15](assets/images/screenshots/Python15.png)

8. **Final figure**
You have now made a customized line chart!

![FinalFigure](assets/images/screenshots/customplot.png)